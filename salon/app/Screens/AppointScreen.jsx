...
//       try {
//         const response = await instance.get(`/bookings/${userId}`);  
...
const AppointScreen = ({ username, userId, image }) => {
  ...
  const handleAppointmentClick = (appointment) => {
    setSelectedAppointment(appointment); // Store the clicked appointment in context
    router.push('Screens/AppointmentDetails'); // Navigate to the details screen
  };
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Appointment List Section */}
      <ScrollView style={styles.appointmentList} showsVerticalScrollIndicator={false}>
        {loading ? (
          <ActivityIndicator size="large" color="#0000ff" />
        ) : (
          appointments.map((appointment) => (
